...
<xsl:stylesheet version="1.1" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:saxon="http://icl.com/saxon" extension-element-prefixes="saxon">
    ...
    <xsl:template name="result">
        ...
        <!--xsl:if test="string-length(@hash) > 0">
            <xsl:text>"lineHash": "</xsl:text><xsl:value-of select="@hash" /><xsl:text>"</xsl:text>
        </xsl:if-->
        <xsl:text> }</xsl:text>
    ...
    <xsl:template name="artifact_location">
        ...
        <xsl:choose>
            <xsl:when test="@locFile">
				<saxon:assign name="locRef" select="@locFile"/>
				<saxon:assign name="locNode" select="/ResultsSession/Locations/Loc[@loc=$locRef]" />
            </xsl:when>
			<xsl:when test="@srcRngFile">
				<saxon:assign name="locRef" select="@srcRngFile"/>
				<saxon:assign name="locNode" select="/ResultsSession/Locations/Loc[@loc=$locRef]" />
			</xsl:when>
		</xsl:choose>
        <xsl:choose>
            <xsl:when test="$locNode/@fsPath">
				<xsl:text>file://</xsl:text>
                <saxon:assign name="uri" select="$locNode/@fsPath"/>
            </xsl:when>
			<xsl:when test="@srcRngFile">
				<xsl:text>file://</xsl:text>
				<saxon:assign name="uri" select="@srcRngFile"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:text>file://</xsl:text>
				<saxon:assign name="uri" select="@locFile"/>
			</xsl:otherwise>
        </xsl:choose>        
        <xsl:call-template name="escape_illegal_chars"><xsl:with-param name="text" select="$uri" /></xsl:call-template>
        <xsl:text>" }</xsl:text>
    </xsl:template>
